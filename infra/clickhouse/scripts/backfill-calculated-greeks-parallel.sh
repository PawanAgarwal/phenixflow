...
CLICKHOUSE_CONNECT_TIMEOUT_SEC="${CLICKHOUSE_CONNECT_TIMEOUT_SEC:-${CALC_GREEKS_CLICKHOUSE_CONNECT_TIMEOUT_SEC:-20}}"
CLICKHOUSE_SEND_TIMEOUT_SEC="${CLICKHOUSE_SEND_TIMEOUT_SEC:-${CALC_GREEKS_CLICKHOUSE_SEND_TIMEOUT_SEC:-600}}"
CLICKHOUSE_RECEIVE_TIMEOUT_SEC="${CLICKHOUSE_RECEIVE_TIMEOUT_SEC:-${CALC_GREEKS_CLICKHOUSE_RECEIVE_TIMEOUT_SEC:-600}}"
...
echo "clickhouse_timeouts_sec: connect=${CLICKHOUSE_CONNECT_TIMEOUT_SEC} send=${CLICKHOUSE_SEND_TIMEOUT_SEC} receive=${CLICKHOUSE_RECEIVE_TIMEOUT_SEC}"
...
for idx in $(seq 0 $((WORKERS - 1))); do
  ...
  (
    ...
    node infra/clickhouse/scripts/backfill-calculated-greeks.js
  ) >"$log_path" 2>&1 &
  pids+=("$!")
done

...
ENRICH_RAM_BUDGET_MB="${ENRICH_RAM_BUDGET_MB:-8192}"
ENRICH_WORKER_OVERHEAD_MB="${ENRICH_WORKER_OVERHEAD_MB:-512}"
...
if ! [[ "$ENRICH_RAM_BUDGET_MB" =~ ^[0-9]+$ ]] || (( ENRICH_RAM_BUDGET_MB < 1024 )); then
  echo "ENRICH_RAM_BUDGET_MB must be an integer >= 1024 (got: $ENRICH_RAM_BUDGET_MB)"
  exit 1
fi

if ! [[ "$ENRICH_WORKER_OVERHEAD_MB" =~ ^[0-9]+$ ]] || (( ENRICH_WORKER_OVERHEAD_MB < 64 )); then
  echo "ENRICH_WORKER_OVERHEAD_MB must be an integer >= 64 (got: $ENRICH_WORKER_OVERHEAD_MB)"
  exit 1
fi
...
ENRICH_WORKER_FOOTPRINT_MB=$(( ENRICH_NODE_MAX_OLD_SPACE_MB + ENRICH_WORKER_OVERHEAD_MB ))
RAM_BUDGET_WORKER_CAP=$(( ENRICH_RAM_BUDGET_MB / ENRICH_WORKER_FOOTPRINT_MB ))
if (( RAM_BUDGET_WORKER_CAP < 1 )); then
  RAM_BUDGET_WORKER_CAP=1
fi

if (( ENRICH_WORKERS > RAM_BUDGET_WORKER_CAP )); then
  echo "Capping workers from $ENRICH_WORKERS to RAM budget cap $RAM_BUDGET_WORKER_CAP"
  ENRICH_WORKERS="$RAM_BUDGET_WORKER_CAP"
fi

ESTIMATED_WORKER_MEMORY_MB=$(( ENRICH_WORKERS * ENRICH_WORKER_FOOTPRINT_MB ))
...
echo "RAM budget: ${ENRICH_RAM_BUDGET_MB}MB"
echo "Per-worker footprint estimate: ${ENRICH_WORKER_FOOTPRINT_MB}MB (heap ${ENRICH_NODE_MAX_OLD_SPACE_MB}MB + overhead ${ENRICH_WORKER_OVERHEAD_MB}MB)"

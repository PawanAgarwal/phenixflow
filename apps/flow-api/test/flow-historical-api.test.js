...
const TEST_RUN_ID = Math.random().toString(36).slice(2, 8).toUpperCase();

function buildTestSymbol(base) {
  return `PF${TEST_RUN_ID}${String(base || '').trim().toUpperCase()}`;
}

const SYMBOL_AAPL = buildTestSymbol('AAPL');
const SYMBOL_IBM = buildTestSymbol('IBM');
const SYMBOL_MSFT = buildTestSymbol('MSFT');
const SYMBOL_K = buildTestSymbol('K');
const SYMBOL_NFLX = buildTestSymbol('NFLX');
const SYMBOL_QQQ = buildTestSymbol('QQQ');
const SYMBOL_TSLA = buildTestSymbol('TSLA');
...
function createDefaultThetaFetch(fetchCalls) {
  return async (url) => {
    ...
    const symbol = parsedUrl.searchParams.get('symbol') || SYMBOL_AAPL;
    const { expirations, strikes, spotSeries } = getSymbolFixture(symbol);

    if (endpoint.includes('/v3/calendar/on_date')) {
      return jsonResponse([
        {
          date: '2026-02-13',
          type: 'open',
...
describe('historical flow API', { timeout: 30000 }, () => {
  let app;
  let tempDir;
  let dbPath;
  let previousDbPath;
  ...
  it('downloads from Theta on cache miss, persists into sqlite, and filters by symbol', async () => {
    const response = await request(app)
      ...
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_AAPL.toLowerCase() });
    ...
    const count = db.prepare('SELECT COUNT(*) AS c FROM option_trades WHERE symbol = ?').get(SYMBOL_AAPL).c;
    const stockRawCount = db.prepare(`
      SELECT COUNT(*) AS c
      FROM stock_ohlc_minute_raw
      WHERE symbol = ? AND trade_date_utc = ?
    `).get(SYMBOL_AAPL, '2026-02-13').c;
  ...
  it('skips Theta call on cache hit for same day/symbol', async () => {
    const response = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_AAPL });
  ...
  it('stores per-minute sigScore component averages in option_symbol_minute_derived', async () => {
    // The primary test symbol data was already synced+enriched by the first test.
    const db = new Database(dbPath, { readonly: true });
    ...
    `).all(SYMBOL_AAPL, FRIDAY_FROM, FRIDAY_TO);
    db.close();
  ...
  it('reports full filtered total even when page limit truncates rows', async () => {
    const response = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_IBM, limit: 1 });
  ...
  it('marks day cache partial when Theta sync fails and retries next call', async () => {
    ...
    global.fetch = async (url) => {
      ...
      if (String(url).includes(`symbol=${SYMBOL_MSFT}`) && failNextMsftFetch) {
    ...
    const first = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_MSFT });
  ...
  it('treats Theta no-data responses as a full zero-row cache', async () => {
    global.fetch = async (url) => {
      fetchCalls.push(String(url));
      if (String(url).includes(`symbol=${SYMBOL_K}`) && String(url).includes('/v3/option/history/trade_quote')) {
        return {
          ok: false,
          status: 472,
    ...
    const response = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_K });
  ...
  it('returns validation error for missing required params', async () => {
    const response = await request(app).get('/api/flow/historical').query({ symbol: SYMBOL_AAPL });
  ...
  it('filters by historical chips and right query params', async () => {
    global.fetch = async (url) => {
      ...
      return {
        ...
        status: 200,
        text: async () => JSON.stringify({
          symbol: [SYMBOL_NFLX, SYMBOL_NFLX, SYMBOL_NFLX],
    ...
    const callsOnly = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_NFLX, chips: 'calls' });
    ...
    const bidOnly = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_NFLX, chips: 'bid' });
  ...
  it('reuses cached supplemental metrics when live supplemental endpoints are unavailable', async () => {
    ...
    try {
      ...
      const response = await request(app)
        .get('/api/flow/historical')
        .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_AAPL, chips: 'otm' });
  ...
  it('uses gov OI reference rows as fallback so vol>oi filter can run without Theta OI endpoint', async () => {
    ...
    `).run('FINRA', 'https://example.gov/oi.csv', '2026-02-13', SYMBOL_AAPL, '2026-02-20', 212.5, 'CALL', 10, '{}');
    db.prepare(`
      INSERT INTO option_open_interest_reference (
        source,
        source_url,
      ...
      ON CONFLICT(source, as_of_date, symbol, expiration, strike, option_right) DO UPDATE SET
        ...
        raw_payload_json = excluded.raw_payload_json,
        ingested_at_utc = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
    `).run('FINRA', 'https://example.gov/oi.csv', '2026-02-13', SYMBOL_AAPL, '2026-02-27', 215, 'CALL', 10, '{}');
    ...
    const response = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_AAPL, chips: 'vol>oi' });
  ...
  it('hydrates spot and oi from Theta endpoints when configured', async () => {
    ...
    try {
      global.fetch = async (url) => {
        ...
        if (endpoint.includes('/v3/stock/snapshot/quote')) {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ symbol: SYMBOL_QQQ, last: 490 }),
          };
        }
      ...
      const response = await request(app)
        .get('/api/flow/historical')
        .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_QQQ, chips: 'otm,vol>oi' });
  ...
  it('treats missing contracts from successful Theta bulk OI response as zero and marks volOi cache full', async () => {
    ...
    try {
      global.fetch = async (url) => {
        ...
        if (endpoint.includes('/v3/option/history/trade_quote')) {
          return {
            ...
            status: 200,
            text: async () => JSON.stringify({
              symbol: [SYMBOL_NFLX, SYMBOL_NFLX],
      ...
      const response = await request(app)
        .get('/api/flow/historical')
        .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_NFLX, chips: 'vol>oi' });
      ...
      `).get(SYMBOL_NFLX, '2026-02-13', 'volOiRatio');
      db.close();
  ...
  it('marks cache partial for explicit limit request, then upgrades to full on no-limit sync', async () => {
    ...
    const limited = await request(app)
      .get('/api/flow/historical')
      .query({ from: FRIDAY_FROM, to: FRIDAY_TO, symbol: SYMBOL_TSLA, limit: 1 });

...
BACKFILL_GAP_TELEMETRY="${BACKFILL_GAP_TELEMETRY:-0}"
...
CLICKHOUSE_ENRICH_GREEKS_SOURCE="${CLICKHOUSE_ENRICH_GREEKS_SOURCE:-index_raw}"
...
if [[ "$BACKFILL_MODE" != "enrich" ]] \
  && (( DOWNLOAD_WORKER_GUARD_ENABLED == 1 )) \
  && (( INPUT_JOB_COUNT >= BACKFILL_DOWNLOAD_WORKER_GUARD_MIN_JOBS )) \
  && (( BACKFILL_WORKERS < BACKFILL_DOWNLOAD_WORKER_GUARD_TARGET )); then
  echo "Download worker guard failed: $INPUT_JOB_COUNT jobs >= guard threshold ${BACKFILL_DOWNLOAD_WORKER_GUARD_MIN_JOBS}, but effective BACKFILL_WORKERS=$BACKFILL_WORKERS < required target ${BACKFILL_DOWNLOAD_WORKER_GUARD_TARGET}."
  echo "Increase workers or tune memory/CPU caps and relaunch."
  exit 1
fi
...
echo "Gap telemetry: $BACKFILL_GAP_TELEMETRY"
...
for idx in $(seq 0 $((BACKFILL_WORKERS - 1))); do
  ...
  (
    ...
    BACKFILL_GAP_TELEMETRY="$BACKFILL_GAP_TELEMETRY" \

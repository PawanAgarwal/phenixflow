...
function selectStrategies(strategyIds) {
  if (!strategyIds?.length || strategyIds.includes('all')) return MULTITIMEFRAME_STRATEGIES;
  const selected = MULTITIMEFRAME_STRATEGIES.filter((strategy) => strategyIds.includes(strategy.id));
  const missing = strategyIds.filter((id) => !MULTITIMEFRAME_STRATEGIES.some((strategy) => strategy.id === id));
  if (missing.length) throw new Error(`unknown_multitimeframe_strategies:${missing.join(',')}`);
  return selected;
}
...
async function main() {
  ...
  const result = await runMultiTimeframePrediction({
    config,
    startDate,
    endDate,
    ...
    onProgress: ({ day, processedDays }) => {
      if (processedDays % 20 === 0) console.log(`processed ${processedDays} multi-timeframe days through ${day.date}`);
    },
  });
  const labelPart = args.label ? `${args.label}-` : '';
  const outPath = artifactPath(`pym-v5-multitimeframe-prediction-${labelPart}${startDate}-${endDate}.json`);

...
function main() {
  ...
  months.forEach((month) => {
    ...
    const predictedRows = testStd.map((row, index) => ({
      ...row,
      bullishProb: bullishProb[index],
      predictedLabel: bullishProb[index] >= BULLISH_THRESHOLD ? 'Bullish' : 'Bearish',
    }));
    monthlyResults.push({
      month,
  ...
  const output = {
    ...
    version: 'v2_binary_directional_walkforward',
    target: {
    ...
    note: 'Monthly walk-forward binary directional model. Uses all prior rows with recency weighting.',
    monthlyResults,
    aggregate: {
  ...
  const outputPath = path.join(OUTPUT_DIR, 'vixregime-monthly-walkforward-v2-binary-directional.json');
